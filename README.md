...
The environment is created in the __environ.py__ file by importing a CSV which contains rows of values (a raster file with pixel values). The __Model_Based_Agents.py__ file allows the agents to be read into the environment. Here we us __Object-Oriented Programming__ which allows multiple instruction to be run simulaneously. 
